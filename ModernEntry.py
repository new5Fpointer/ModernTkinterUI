    ...
    def delete(self, first, last=None):
        ...
        if start is not None and end is not None:
            ...
            for rect in self._selection_rects:
                super().delete(rect)  # 使用Canvas的原始delete方法
...
class DemoApp:
    ...
    def _create_header(self, parent):
        ...
        tk.Label(header_frame, text="Modern Entry", font=("Segoe UI", 16, "bold"),
                 fg=SUCCESS_COLOR, bg=BG_COLOR).pack(side="left")
        tk.Label(header_frame, text="Smooth cursor animation", font=("Segoe UI", 10),
                 fg=PLACEHOLDER_COLOR, bg=BG_COLOR).pack(side="left", padx=(10, 0))
        tk.Label(header_frame, text="(Now with selection support!)", font=("Segoe UI", 9),
    ...
    def _create_labeled_entry(self, parent, label_text, placeholder, row, fixed_size=True, max_length=MAX_TEXT_LENGTH):
        ...
        entry = ModernEntry(parent, width=DEFAULT_WIDTH, height=DEFAULT_HEIGHT, placeholder=placeholder,
    ...
    def _create_buttons(self, parent):
        ...
        submit_btn = tk.Button(button_frame, text="Submit", font=("Segoe UI", 10, "bold"),
                               bg=BUTTON_BG_COLOR, fg=BUTTON_TEXT_COLOR,
                               activebackground=BUTTON_ACTIVE_BG, activeforeground=BUTTON_ACTIVE_FG,
                               relief="flat", padx=20, pady=6, command=self.submit_form)
        submit_btn.pack(side="right", padx=BUTTON_PAD_X)

        clear_btn = tk.Button(button_frame, text="Clear All", font=("Segoe UI", 10),
                              bg=BUTTON_SECONDARY_BG, fg=BUTTON_SECONDARY_FG,
                              activebackground=BUTTON_SECONDARY_ACTIVE_BG,
                              activeforeground=BUTTON_ACTIVE_FG, relief="flat", padx=20, pady=6,
                              command=self.clear_form)
        clear_btn.pack(side="right")

        copy_btn = tk.Button(button_frame, text="Copy Selected", font=("Segoe UI", 10),
                             bg=BUTTON_SECONDARY_BG, fg=BUTTON_SECONDARY_FG,
                             activebackground=BUTTON_SECONDARY_ACTIVE_BG,
                             activeforeground=BUTTON_ACTIVE_FG, relief="flat", padx=20, pady=6,
    ...
    def submit_form(self):
        ...
        success = tk.Label(self.root, text="✓ Form submitted successfully!",
    ...
    def copy_selected(self):
        ...
        if isinstance(focus, ModernEntry):
            ...
            if selected_text:
                ...
                success = tk.Label(self.root, text="✓ Copied to clipboard!",
